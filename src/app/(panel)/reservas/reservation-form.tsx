...
function selectTariffPlanForDate(
  plans: Props["tariffCatalogs"],
  dateKey: string,
...
function resolveAutoRateCodeForDate(plans: Props["tariffCatalogs"], dateTime: string): string {
...
function isRateCodeActiveForDate(plans: Props["tariffCatalogs"], rateCode: string, dateTime: string): boolean {
...
function calculateTariffAmountFromPlans(input: {
  ...
  for (const [planId, segmentDays] of planDays.entries()) {
    ...
    if (!plan) return { found: false, amount: 0 };
    const base = resolveTariffAmountForPlanDays({
      brackets: plan.brackets,
      prices: plan.prices,
      groupCode: input.groupCode,
...
function getAvailablePlatesForGroup(input: {
  ...
  if (!requestedGroup || !input.deliveryAt || !input.pickupAt) {
    return [];
  }
...
export function ReservationForm({
  ...
  const [activeBottomTab, setActiveBottomTab] = useState<"notas-publicas" | "notas-privadas" | "seguros" | "extras" | "conductores">(
    "notas-publicas",
  );
  const parsedAdditionalDrivers = parseAdditionalDrivers(String(initialValues?.additionalDrivers ?? ""));
  const initialName = String(initialValues?.customerName ?? (initialClient ? clientDisplayName(initialClient) : ""));
  const [clientLookup, setClientLookup] = useState("");
  const [customerId, setCustomerId] = useState(String(initialValues?.customerId ?? initialClient?.clientCode ?? ""));
  const [customerName, setCustomerName] = useState(initialName);
  const [customerCompany, setCustomerCompany] = useState(String(initialValues?.customerCompany ?? initialClient?.companyName ?? ""));
  const [customerCommissioner, setCustomerCommissioner] = useState(String(initialValues?.customerCommissioner ?? initialClient?.commissionerName ?? ""));
  const [salesChannel, setSalesChannel] = useState(String(initialValues?.salesChannel ?? initialClient?.acquisitionChannel ?? ""));
  const initialBranchCode =
    initialValues?.branchDelivery && branches.some((branch) => branch.code === initialValues.branchDelivery)
      ? initialValues.branchDelivery
      : defaultBranchCode && branches.some((branch) => branch.code === defaultBranchCode)
        ? defaultBranchCode
        : (branches[0]?.code ?? "");
  const [branchDelivery, setBranchDelivery] = useState(initialBranchCode);
  const [pickupBranch, setPickupBranch] = useState(String(initialValues?.pickupBranch ?? initialBranchCode));
  ...
  const [blockPlateForReservation, setBlockPlateForReservation] = useState(Boolean(initialValues?.blockPlateForReservation));
  ...
  const [selectedInsuranceId, setSelectedInsuranceId] = useState(insuranceOptions[0]?.id ?? "");
  const [insuranceUnitsInput, setInsuranceUnitsInput] = useState("1");
  ...
  const insuranceAmount = useMemo(() => selectedInsurances.reduce((sum, item) => sum + item.amount, 0), [selectedInsurances]);
  ...
  const availablePlatesForGroup = useMemo(() => {
    return getAvailablePlatesForGroup({
  ...
  const selectedAssignedPlateMissing = useMemo(
    () => Boolean(assignedPlate && !availablePlatesForGroup.includes(assignedPlate)),
    [assignedPlate, availablePlatesForGroup],
  );
  ...
  const selectedInsuranceOption = useMemo(
    () => insuranceOptions.find((item) => item.id === selectedInsuranceId) ?? null,
  ...
  const billedDays = useMemo(
    () => computeBilledDaysBy24h(deliveryAt, pickupAt, globalCourtesyHours),
    [deliveryAt, globalCourtesyHours, pickupAt],
  );
  const selectedInsuranceUnits = useMemo(() => {
    if (!selectedInsuranceOption) return 0;
    return Math.max(1, parseNumberInput(insuranceUnitsInput));
  }, [insuranceUnitsInput, selectedInsuranceOption]);
  const selectedInsuranceChargeDays = useMemo(() => {
    if (!selectedInsuranceOption || selectedInsuranceOption.priceMode !== "POR_DIA") return 0;
    return selectedInsuranceOption.maxDays > 0 ? Math.min(billedDays, selectedInsuranceOption.maxDays) : billedDays;
  }, [billedDays, selectedInsuranceOption]);
  const selectedInsuranceUnitPrice = selectedInsuranceOption ? selectedInsuranceOption.unitPrice.toFixed(2) : "";
  const selectedInsuranceTotalPrice = selectedInsuranceOption
    ? (
        selectedInsuranceOption.priceMode === "POR_DIA"
          ? selectedInsuranceOption.unitPrice * selectedInsuranceUnits * selectedInsuranceChargeDays
          : selectedInsuranceOption.unitPrice * selectedInsuranceUnits
      ).toFixed(2)
    : "";
  ...
  }, [discountAmount, effectiveBaseAmountNumber, insuranceAmount, selectedExtras, fuelAmount, penaltiesAmount]);
  ...
  function addInsuranceLine() {
    const insurance = insuranceOptions.find((item) => item.id === selectedInsuranceId);
    if (!insurance) return;
    const rawUnits = Math.max(1, parseNumberInput(insuranceUnitsInput));
    const units = rawUnits;
    const chargeDays = insurance.priceMode === "POR_DIA" ? (insurance.maxDays > 0 ? Math.min(billedDays, insurance.maxDays) : billedDays) : 0;
    const amount = insurance.priceMode === "POR_DIA" ? insurance.unitPrice * units * chargeDays : insurance.unitPrice * units;
    setSelectedInsurances((current) => [
      ...current,
      {
        extraId: insurance.id,
        code: insurance.code,
        name: insurance.name,
        priceMode: insurance.priceMode,
  ...
  function removeInsuranceLine(index: number) {
    setSelectedInsurances((current) => current.filter((_, idx) => idx !== index));
  }
    ...
    <form action={action} className="stack-md">
      <div className="reservation-create-layout">
        <div className="stack-md">
          ...
          <section className="card-muted stack-sm">
            ...
            <div className="form-grid">
              <label>
                Sucursal entrega *
                <select name="branchDelivery" value={branchDelivery} onChange={(e) => handleBranchDeliveryChange(e.target.value)} required disabled={!canWrite}>
                  <option value="">Selecciona</option>
                  {branches.map((branch) => (
              ...
              <label>
                Fecha/hora entrega *
                <input name="deliveryAt" type="datetime-local" value={deliveryAt} onChange={(e) => handleDeliveryAtChange(e.target.value)} required disabled={!canWrite} />
              </label>
              <label>
                Vuelo entrega
                <input name="deliveryFlightNumber" defaultValue={initialValues?.deliveryFlightNumber ?? ""} disabled={!canWrite} />
              </label>
              <label>
                Sucursal recogida
                <select name="pickupBranch" value={pickupBranch} onChange={(e) => setPickupBranch(e.target.value)} disabled={!canWrite}>
                  <option value="">Selecciona</option>
                  {branches.map((branch) => (
              ...
              <label>
                Fecha/hora recogida *
                <input name="pickupAt" type="datetime-local" value={pickupAt} onChange={(e) => handlePickupAtChange(e.target.value)} required disabled={!canWrite} />
              </label>
              <label>
                Vuelo recogida
                <input name="pickupFlightNumber" defaultValue={initialValues?.pickupFlightNumber ?? ""} disabled={!canWrite} />
              </label>
              ...
              <label className="field-compact">
                Estado
                <select name="reservationStatus" defaultValue={initialValues?.reservationStatus ?? "CONFIRMADA"} disabled={!canWrite}>
          ...
          <section className="card-muted stack-sm">
            ...
            <div className="form-grid">
              <label className="field-compact">
                Grupo reservado *
                <select name="billedCarGroup" required value={billedCarGroup} onChange={(e) => handleBilledGroupChange(e.target.value)} disabled={!canWrite}>
                  <option value="">Selecciona</option>
              ...
              <label className="field-compact">
                Vehículo disponible
                <select name="assignedPlate" value={assignedPlate} onChange={(e) => handleAssignedPlateChange(e.target.value)} disabled={!canWrite || !assignedVehicleGroup || !deliveryAt || !pickupAt}>
                  <option value="">Selecciona</option>
                  {selectedAssignedPlateMissing ? (
                    <option value={assignedPlate} hidden>
                      {assignedPlate}
                    </option>
                  ) : null}
              ...
              <label className="field-compact">
                Grupo entregado
                <select name="assignedVehicleGroup" value={assignedVehicleGroup} onChange={(e) => handleAssignedVehicleGroupChange(e.target.value)} disabled={!canWrite}>
                  <option value="">Selecciona</option>
                  {groupOptions.map((group) => (
              ...
              <label>
                ...
                <select name="blockPlateForReservation" value={blockPlateForReservation ? "true" : "false"} onChange={(e) => setBlockPlateForReservation(e.target.value === "true")} disabled={!canWrite}>
        ...
        <aside className="price-side-card">
          ...
          <label>
            Tarifa
            <select value={displayedRateCode} onChange={(e) => handleAppliedRateChange(e.target.value)} disabled={!canWrite}>
          ...
          <label>
            Seguros
            <input name="insuranceAmountPreview" type="number" step="0.01" value={insuranceAmount.toFixed(2)} readOnly />
          </label>
          <label>
            Extras
      ...
      <section className="card-muted stack-sm">
          <div className="table-header-row">
            ...
            <button
              type="button"
              className={activeBottomTab === "seguros" ? "primary-btn" : "secondary-btn"}
              onClick={() => setActiveBottomTab("seguros")}
            >
              Seguros
            </button>
            <button
        ...
        {activeBottomTab === "notas-publicas" ? (
          <div className="form-grid">
            <textarea className="col-span-2" name="publicNotes" rows={3} defaultValue={initialValues?.publicNotes ?? ""} disabled={!canWrite} />
          </div>
        ) : null}

        {activeBottomTab === "notas-privadas" ? (
          <div className="form-grid">
            <textarea className="col-span-2" name="privateNotes" rows={3} defaultValue={initialValues?.privateNotes ?? ""} disabled={!canWrite} />
          </div>
        ) : null}

        {activeBottomTab === "seguros" ? (
          <div className="form-grid">
              <div className="extras-inline-row col-span-2">
                <label className="extras-inline-main">
                  Seguro
                  <select value={selectedInsuranceId} onChange={(e) => setSelectedInsuranceId(e.target.value)} disabled={!canWrite}>
                    <option value="">Selecciona</option>
                    {insuranceOptions.map((insurance) => (
                      <option key={insurance.id} value={insurance.id}>
                        {insurance.code} - {insurance.name} ({insurance.priceMode === "POR_DIA" ? "día" : "fijo"}) {insurance.unitPrice.toFixed(2)}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="extras-inline-mini">
                  Unidades
                  <input value={insuranceUnitsInput} onChange={(e) => setInsuranceUnitsInput(e.target.value)} type="number" min={1} disabled={!canWrite} />
                </label>
                <label className="extras-inline-mini">
                  Precio ud.
                  <input value={selectedInsuranceUnitPrice} readOnly />
                </label>
                <label className="extras-inline-mini">
                  Total
                  <input value={selectedInsuranceTotalPrice} readOnly />
                </label>
                <button type="button" className="secondary-btn extras-inline-add" onClick={addInsuranceLine} disabled={!canWrite || !selectedInsuranceId}>
                  Añadir seguro
                </button>
              </div>
              {selectedInsurances.length > 0 ? (
                    ...
                    <tbody>
                      {selectedInsurances.map((item, index) => (
                        <tr key={`${item.extraId}-${index}`}>
                          ...
                          <td>
                            <button type="button" className="secondary-btn" onClick={() => removeInsuranceLine(index)} disabled={!canWrite}>
                              Quitar
                            </button>
                          </td>
                        </tr>
                ...
                <p className="col-span-2 muted-text">Sin seguros añadidos.</p>
              )}
            </div>
          ) : null}
        ...
        {activeBottomTab === "extras" ? (
          ...
          <div className="table-header-row">
            <button className="primary-btn" type="submit" disabled={!canWrite}>
              {initialValues?.reservationId ? "Guardar cambios" : "Guardar reserva"}
            </button>
            <button
              ...
              onClick={() => {
                setClientLookup("");
                setCustomerId(String(initialClient?.clientCode ?? ""));
                ...
                setSelectedInsuranceId(insuranceOptions[0]?.id ?? "");
                setInsuranceUnitsInput("1");
            ...
            <button
              ...
              onClick={() => {
                if (!priceLocked) {
                  setBaseAmount(effectiveBaseAmount);
                }
        ...
        <input type="hidden" name="insuranceAmount" value={insuranceAmount.toFixed(2)} readOnly />
        <input
          type="hidden"
          name="selectedInsurancePayload"
          value={JSON.stringify(selectedInsurances.map((item) => ({ extraId: item.extraId, units: item.units })))}
          readOnly
        />
        <input
          type="hidden"
        ...
        <input
          type="hidden"
          name="selectedDiscountsPayload"
          value={JSON.stringify(computedDiscountRows.map((item) => ({ percent: item.percent })))}
          readOnly
        />
        ...
        {initialValues?.reservationId ? <input type="hidden" name="reservationId" value={initialValues.reservationId} readOnly /> : null}
